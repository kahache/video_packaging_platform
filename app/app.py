...
__requirements__ = "SQL-Alchemy, MySQL, Flask-SQLAlchemy, database.py, " \
                   "models.py, video_ops.py, main_ops.py"
...
def file_received():
    ...
    if file_operation[0] == 1:
        return render_template("success.html", name=file_operation[1],
                               input_content_id=file_operation[2])
    else:
...
def background_package():
    """When called with JSON, it will generate background operations with
    video files inside internal storage
    It works with nested operations, if one operation is successful,
    then starts the next.
    """
    file_packaging = Main_ops.package()
    if file_packaging[0] == 1:
        data_set = {"packaged_content_id": [file_packaging[2]],
                    "url": [file_packaging[1]]}
        return (data_set)
    else:
        return ("ERROR - Check command line")
...
def consult_status(packaged_content_id):
    """
    This method is called for big video files, to check the status of the
    operations.
    As the video files' operations update the Database with the status of the
    file, by knowing that status
...
def get_videos():
    """
    This method is to start serving files in the same server. It's just a
    quick way to access
...
if __name__ == '__main__':
    print("THIS IS THE NEW VERSION", flush=True)
