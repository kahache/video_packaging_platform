...
This project was originally written for Python 3.6 and SQLAlchemy 1.x. It has been brought up to a current stack:

- **Python 3.12** base image (`Dockerfile`) with **pinned dependencies** in `app/requirements.txt`.
- **Migrated to SQLAlchemy 2.x**: classical `mapper()` → `registry().map_imperatively()`; removed `autoload=True` reflection (the `uploaded_videos` table is now defined once in `models.py`); `Table.select(whereclause)` → `select().where(...)`; explicit `con.commit()` on writes (2.x dropped implicit autocommit); and the engine URL uses the bundled **PyMySQL** driver (`mysql+pymysql://`).
- **Bug fixes**: `consult_status` no longer crashes on a non-`Ready` item (int/str concatenation); `packaged_content_id` is now a sequential `MAX+1` value instead of a random 0–100 (which collided against the `UNIQUE` constraint); the `models.py` constructor now stores its arguments; and working-directory cleanup in `video_ops.py` runs in a `finally` block. A latent `engine` name shadowing in `main_ops.py` (from `from sqlalchemy import *`) was also fixed.
- **docker-compose**: added a MySQL **healthcheck** so the app waits for the database to be ready, and removed an invalid `MYSQL_USER: root` that prevented the DB container from initializing.
- **New end-to-end tests** under `tests/e2e/` (see *Running the tests*).
...
```
(On older installs use the legacy `docker-compose up --build`.) The app container waits for MySQL to pass its healthcheck before starting, so the first request won't hit an uninitialized database.
...
```
Python 3.12 (the Docker image uses python:3.12; 3.6 no longer works with the pinned deps)
MySQL Server
...
**_packaged_content_id_** : ID for a file that is in the process of being packaged. This is a sequential id (the current maximum plus one) generated by our main App, so it stays unique against the `UNIQUE` constraint. (It used to be a random 0–100 value, which could collide.)
...
For the docker, the database is declared in **/db/init.sql** (mounted into the MySQL container's `docker-entrypoint-initdb.d/`, so the schema is created on first boot — the app does not call `create_all()` itself).
...
In addition to the dialog-based tests above, there is a dedicated end-to-end suite:

| Test | What it exercises | Needs Docker/MySQL? |
|------|-------------------|---------------------|
| `tests/e2e/pipeline_e2e.py` | The **real** `app/video_ops.py` code: ingest → fragment → encrypt → dash, asserting each stage succeeds and every output file (fragmented mp4, `_enc.mp4`, `dash/stream.mpd`) exists | **No** — `video_ops` is database-free; it only needs the Bento4 binaries in `bin/` |
| `tests/e2e/http_e2e.py` | The **live REST API**: `POST /success` → `POST /packaged_content` → `GET /packaged_content_id/<id>` | **Yes** — run `docker compose up` first |

```
bash tests/e2e/get_sample_video.sh        # fetch a small sample into tests/TEST_VIDEOS/
pip3 install -r app/requirements.txt

# fast path, no Docker needed:
python3 tests/e2e/pipeline_e2e.py
# or point it at your own file:
python3 tests/e2e/pipeline_e2e.py /path/to/video.mp4

# full stack over HTTP:
docker compose up --build
python3 tests/e2e/http_e2e.py
```

The video track id is auto-detected during ingest, so any standard H.264 mp4 works. Exit codes: `0` pass, `1` fail, `2` skipped (no sample video). See `tests/e2e/README.md` for environment variables.
...
* [Python 3.12](https://www.python.org/downloads/) - Most programming language used
* [Flask](https://flask.palletsprojects.com/) - Framework used to generate the API
* [SQLAlchemy 2.x](https://docs.sqlalchemy.org/) / [Flask-SQLAlchemy](https://flask-sqlalchemy.palletsprojects.com/) - Library used as ORM
* [PyMySQL](https://pymysql.readthedocs.io/) - Pure-Python MySQL driver used by the engine URL
